...
from rag_agent_elastic import graph
from rag_agent import graph as graph_agent
from rag_hyde import hyde
from rag_multi_query import multi
from rag_hyde_or_multi import app as app_hyde_or_multi
from rag_single_or_decom import router_app as app_single_or_decom
from rag_decom_feedback import orchestrator_worker as app_decom_feedback
from rag_filter_feedback import graph as app_filter_feedback
...
async def chat_endpoint(req: ChatRequest):
    ...
    try:
        ...
        result = app_filter_feedback.invoke(input_state, config=config)
...
async def feedback_endpoint(req: FeedbackRequest):
    ...
    try:
        resume_value = {"action": req.action, "query": req.query, "filter": req.filter}
        # We invoke with Command(resume=value)
        result = app_filter_feedback.invoke(Command(resume=resume_value), config=config)
